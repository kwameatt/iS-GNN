...
class D_GCN(nn.Module):
    ...
    def forward(self, X, A_q, A_h):
        ...
        x0_all = X.permute(1, 2, 0).reshape(num_node, input_size * batch_size)
        x = torch.unsqueeze(x0_all, 0)
        for support in supports:
            x0 = x0_all                      # <-- reset for each support
            x1 = support @ x0
            x  = self._concat(x, x1)
            for k in range(2, self.orders + 1):
                x2 = 2 * (support @ x1) - x0
...
class IGNNK_ModOptionB(nn.Module):
    """
    IGNNK modified for separating target channels and auxiliary node features.
    """
    def __init__(self, h_total, h_target, z, k, dropout=0.0):
        """
        h_total  : total input channels (target + auxiliary features)
        h_target : number of channels to reconstruct (e.g., 2 for encoded shmax values)
        z        : hidden dimension
        k        : diffusion steps
        """
        super(IGNNK_ModOptionB, self).__init__()
        self.h_total = h_total
        self.h_target = h_target
        self.z = z
        self.k = k
        self.time_dimension = h_total
        self.drop1 = nn.Dropout(dropout) if dropout > 0 else nn.Identity()
        self.drop2 = nn.Dropout(dropout) if dropout > 0 else nn.Identity()

        self.GNN1 = D_GCN(self.h_total, self.z, self.k, activation="selu")
        self.GNN2 = D_GCN(self.z, self.z, self.k, activation="selu")
        self.GNN3 = D_GCN(self.z, self.h_target, self.k, activation='linear')  # only predict target channels

    def forward(self, X, A_q, A_h):
        """
        Forward pass
        X   : (batch, h_total, n_nodes)
        A_q : random walk matrix (n_nodes x n_nodes)
        A_h : reversed random walk matrix (n_nodes x n_nodes)
        Returns:
        X_pred : (batch, h_target, n_nodes)  → only cos2θ, sin2θ predicted
        """
        X_S = X.permute(0, 2, 1)       # (batch, n_nodes, h_total)
        X_s1 = self.GNN1(X_S, A_q, A_h)
        X_s2 = self.GNN2(self.drop1(X_s1), A_q, A_h) + X_s1
        X_s3 = self.GNN3(self.drop2(X_s2), A_q, A_h)
        X_pred = X_s3.permute(0, 2, 1)  # (batch, h_target, n_nodes)
        return X_pred
