...
def generate_shmax_train_data(anchor_df_path: str, grid_gdf_path: Union[str, gpd.GeoDataFrame]):
    ...
    anchor_df = pd.read_csv(anchor_df_path, encoding='ISO-8859-1')
    assert {'ID','LON','LAT','DEPTH','QUALITY','AZI','DIST','REGIME_1'}.issubset(anchor_df.columns)
    ...
    anchor_gdf = anchor_gdf[~(anchor_gdf["QUALITY"] == 'E')]
    anchor_gdf = anchor_gdf[~(anchor_gdf["QUALITY"] == 'D')]
    anchor_gdf = anchor_gdf[~(anchor_gdf["REGIME_1"] == 'U')]
    ...
    grid_gdf = quality_assignment(
        anchor_gdf, grid_gdf,
        R_km=200, cutoff_frac=0.10, weight_mode="inverse"
    )
    ...
    #keep only high quality quality data. 
    grid_gdf = grid_gdf[~(grid_gdf['QUALITY'] == 'E')]
    grid_gdf = grid_gdf[~(grid_gdf['QUALITY'] == 'D')]
    grid_gdf = grid_gdf[~(grid_gdf['QUALITY'] == 'Xne')]
    grid_gdf = grid_gdf[~(grid_gdf['QUALITY'] == 'Xmi')]
    grid_gdf = grid_gdf[~(grid_gdf['QUALITY'] == 'Xru')]
    grid_gdf = grid_gdf[grid_gdf["QUALITY"] != "U"]
    anchor_gdf = anchor_gdf[~(anchor_gdf['QUALITY'] == 'Xne')]
    anchor_gdf = anchor_gdf[~(anchor_gdf['QUALITY'] == 'Xmi')]
    anchor_gdf = anchor_gdf[~(anchor_gdf['QUALITY'] == 'Xru')]    
