...
LEN_MAX=0.30   # inches: length when MAG=1 (so span = LEN_MAX - LEN_MIN)
...
awk -F"$FS" -v LEN_MIN="$LEN_MIN" -v LEN_MAX="$LEN_MAX" '
  ...
  NR>1 && $2!="" && $3!="" && $10!="" && $12!="" {
    ...
    print lon, lat, azi,       len
    print lon, lat, azi+180.0, len
  }
' "$data0" > "$bars0"
...
gmt makecpt -Cviridis -T0/1/0.1

# GRID: azimuth+length (cols 3–4), color from col 5 (+z)
gmt plot "$bars0" -SV0.0c+e0c -W0.8p,darkgreen -Gdarkgreen@60 -L
